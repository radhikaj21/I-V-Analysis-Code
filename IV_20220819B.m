...
for i = 1:length(fileList)
    ...
    range_Iss = curr(Iss_start:Iss_end,:); %selects part of current data
    mean_range_Iss = mean(range_Iss); %makes an averageof the current range
    mrangeIlist{i}=mean_range_Iss
...
%Build IV curve 
    ...
    Input_R = (abs(mean_range_Vss)/abs(mean_range_Iss)) %Input resistance is the quotient of voltage divided by current
    Input_R_list(i)= Input_R
